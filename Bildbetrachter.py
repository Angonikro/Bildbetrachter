...
class ImageViewerApp:
    ...
    def show_manual(self):
        ...
        guide_de = """BILDBETRACHTER v0.1.11 – ANLEITUNG
...
Info
Bildbetrachter v0.1.11
By Goldisoft 2026
"""
        guide_en = """IMAGE VIEWER v0.1.11 – USER GUIDE
...
"""
    ...
    def crop_finish_event(self, event):
        ...
        left = max(0, int(left / self.zoom_factor))
        top = max(0, int(top / self.zoom_factor))
        right = min(self.image.width, int(right / self.zoom_factor))
        bottom = min(self.image.height, int(bottom / self.zoom_factor))
